...
class Graph(defaultdict):
    ...
    def make_undirected(self):
        ...
        t1 = time()
        logging.info('make_directed: added missing edges {}s'.format(t1-t0))
...
def parse_adjacencylist(f):
    ...
    for el in f:
        if el and el[0] != "#":
            introw = [int(x) for x in el.strip().split()]
            row = [introw[0]]
...
def parse_adjacencylist_unchecked(f):
    ...
    for el in f:
        if el and el[0] != "#":
            adjlist.extend([[int(x) for x in el.strip().split()]])
    return adjlist
...
def load_edgelist(file_, undirected=True):
    G = Graph()
    logging.info(f"Load edges from file: {file_}")
    with open(file_) as f:
        for el in f:
            if len(el.strip().split()[:2]) > 1:
                x, y = el.strip().split()[:2]
                x = int(x)
                y = int(y)
                G[x].append(y)
                if undirected:
